...
const BookingFlow = ({ service, specialists, bookings, services, onConfirm, onExit, language, userId, initialSpecialistId }) => {
    const { t, lt } = useLocalization(language);
    const [step, setStep] = useState(initialSpecialistId ? 'datetime' : 'specialist');
    const [selectedSpecialistId, setSelectedSpecialistId] = useState(initialSpecialistId || 'any');
    const [selectedDate, setSelectedDate] = useState('');
    const [selectedTime, setSelectedTime] = useState('');

    const handleSpecialistSelect = (id) => {
        ...
        <div className="flex flex-col h-full bg-gray-50">
            ...
            <main className="flex-grow p-4">
                ...
                {step === 'confirm' && (
                    <ConfirmStep 
                        service={service}
                        specialist={specialists.find(s => s.id === selectedSpecialistId) || (selectedSpecialistId === 'any' ? {name: t('anySpecialist')} : undefined)}
                        date={selectedDate}
                        time={selectedTime}
...
const SpecialistStep = ({ specialists, onSelect, t }) => (
...
const DateTimeStep = ({ service, services, specialists, bookings, selectedSpecialistId, onSelect, t }) => {
    ...
    const TimeOfDayFilter = () => (
        <div className="my-4">
            ...
            <div className="flex justify-between space-x-2">
                {(['anyTime', 'morning', 'afternoon', 'evening']).map(period => (
                    <button
                        key={period}
...
const ConfirmStep = ({ service, specialist, date, time, onConfirm, t, lt }) => {
    return (
