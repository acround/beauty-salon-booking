...
import { User, Language, Service, Specialist, Booking, Publication, LocalizedString, Role, Review, ScheduleChangeRequest, ScheduleChangeRequestStatus, PublicationType } from '../types';
...
interface AdminScreenProps {
    ...
    setSpecialists: React.Dispatch<React.SetStateAction<Specialist[]>>;
    bookings: Booking[];
    setBookings: React.Dispatch<React.SetStateAction<Booking[]>>;
    news: Publication[];
    setNews: React.Dispatch<React.SetStateAction<Publication[]>>;
    scheduleRequests: ScheduleChangeRequest[];
    onScheduleRequestUpdate: (requestId: string, status: 'approved' | 'rejected') => void;
    onAddScheduleRequest: (newRequest: Omit<ScheduleChangeRequest, 'id'>) => void;
}

type AdminView = 'dashboard' | 'specialists' | 'services' | 'schedule' | 'publications' | 'stats' | 'reviews';

const emptyLocalizedString: LocalizedString = { ru: '', en: '', sr: '' };
...
const DashboardView: React.FC<Pick<AdminScreenProps, 'services' | 'specialists' | 'bookings' | 'news'> & { t: (key: any) => string }> = ({ services, specialists, bookings, news, t }) => {
...
const AdminNavLink: React.FC<{onClick: ()=>void, label: string, active: boolean}> = ({onClick, label, active}) => (
    <button 
        onClick={onClick}
...
const SpecialistManagement: React.FC<AdminScreenProps> = ({ specialists, setSpecialists }) => {
    const [editingSpecialist, setEditingSpecialist] = useState<Specialist | null>(null);

    const handleSave = (spec: Specialist) => {
    ...
    return(
        <div>
            ...
            {!editingSpecialist ? (
                <>
                <button onClick={() => setEditingSpecialist({} as Specialist)} className="mb-4 bg-brand-primary text-white font-bold py-2 px-4 rounded hover:bg-opacity-90">
                    Add Specialist
                </button>
...
const SpecialistForm: React.FC<{specialist: Specialist, onSave: (spec: Specialist) => void, onCancel: () => void, setSpecialists: React.Dispatch<React.SetStateAction<Specialist[]>>}> = ({ specialist, onSave, onCancel, setSpecialists }) => {
    const [formState, setFormState] = useState<Specialist>(specialist);
    ...
    const handleChange = <T,>(field: keyof Specialist, value: T) => {
        setFormState(prev => ({...prev, [field]: value}));
    };

    const handleReviewAction = (reviewId: string, action: 'approve' | 'unapprove' | 'reject') => {
...
const ServiceManagement: React.FC<AdminScreenProps> = ({ services, setServices, specialists }) => {
    ...
    return(
        <div>
            ...
            {!editingService ? (
                <>
                <button onClick={() => setEditingService({} as Service)} className="mb-4 bg-brand-primary text-white font-bold py-2 px-4 rounded hover:bg-opacity-90">
                    Add Service
                </button>
                <div className="space-y-2">
...
const ServiceForm: React.FC<{service: Service, onSave: (s: Service) => void, onCancel: () => void, specialists: Specialist[]}> = ({ service, onSave, onCancel, specialists }) => {
    const [formState, setFormState] = useState<Service>(service);

    const handleChange = <T,>(field: keyof Service, value: T) => {
        setFormState(prev => ({...prev, [field]: value}));
    };

    const handleSpecialistToggle = (specId: string) => {
...
const ScheduleManagement: React.FC<AdminScreenProps & { t: (key: any) => string }> = (props) => {
    ...
    const handleDirectScheduleUpdate = (date: string, newSlots: string[]) => {
    ...
    const handleRequestSubmit = () => {
        ...
        const newRequest: Omit<ScheduleChangeRequest, 'id'> = {
...
}> = ({ displayDate, selectedDate, onMonthChange, onDateSelect, workSchedule }) => {
...
const ScheduleEditor: React.FC<{
    ...
    selectedDate: string;
    schedule: string[];
    onUpdate: (date: string, newSlots: string[]) => void;
    t: (key: any) => string;
}> = ({ isAdmin, selectedDate, schedule, onUpdate, t }) => {
...
const PublicationManagement: React.FC<AdminScreenProps & { t: (key: any) => string }> = ({ news, setNews, t }) => {
    const [editingPublication, setEditingPublication] = useState<Publication | null>(null);

    const handleSave = (publication: Publication) => {
    ...
    return (
        <div>
            ...
            {!editingPublication ? (
                <>
                    <button onClick={() => setEditingPublication({} as Publication)} className="mb-4 bg-brand-primary text-white font-bold py-2 px-4 rounded hover:bg-opacity-90">
                        {t('addPublication')}
                    </button>
                    <div className="space-y-2">
                        {news.map(pub => (
...
const PublicationForm: React.FC<{
    ...
    onCancel: () => void;
    t: (key: any) => string;
}> = ({ publication, onSave, onCancel, t }) => {
    const [formState, setFormState] = useState<Publication>(publication);

    const handleChange = <T,>(field: keyof Publication, value: T) => {
        setFormState(prev => ({ ...prev, [field]: value }));
    };

    const handlePromoPeriodChange = (part: 'start' | 'end', value: string) => {
...
const StatisticsView: React.FC<Pick<AdminScreenProps, 'bookings' | 'services' | 'specialists'> & { t: (key: any) => string }> = ({ bookings, services, specialists, t }) => {
    const [startDate, setStartDate] = useState('');
    const [endDate, setEndDate] = useState('');
    const [filterType, setFilterType] = useState<'service' | 'specialist'>('specialist');
            ...
            <div className="p-4 bg-white rounded-lg shadow mb-6 space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div>
                        ...
                        <select value={filterType} onChange={e => { setFilterType(e.target.value as any); setSelectedId(''); setReport(null); }} className="w-full p-2 border rounded-md mt-1 bg-white">
                    ...
                    <div>
                        ...
                        <select value={selectedId} onChange={e => setSelectedId(e.target.value)} className="w-full p-2 border rounded-md mt-1 bg-white" disabled={!filterType}>
                            ...
                            {options.map(option => (
                                <option key={option.id} value={option.id}>{filterType === 'specialist' ? (option as Specialist).name : (option as Service).name.ru}</option>
                            ))}
                        </select>
                    </div>
                 </div>
...
const ReviewManagementView: React.FC<AdminScreenProps & { t: (key: any) => string }> = ({ specialists, setSpecialists, t }) => {
    ...
    const handleReviewAction = (specialistId: string, reviewId: string, action: 'approve' | 'unapprove' | 'reject') => {
...
const ReviewCard: React.FC<{
    review: Review & { specialistId: string; specialistName: string; };
    onAction: (specialistId: string, reviewId: string, action: 'approve' | 'unapprove' | 'reject') => void;
    t: (key: any) => string;
}> = ({ review, onAction, t }) => (
